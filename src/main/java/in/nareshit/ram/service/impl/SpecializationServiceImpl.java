package in.nareshit.ram.service.impl;

import java.util.List;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import in.nareshit.ram.entity.Specialization;
import in.nareshit.ram.exception.SpecializationNotFoundExcetion;
import in.nareshit.ram.repo.SpecializationRepository;
import in.nareshit.ram.service.ISpecializationService;

@Service
public class SpecializationServiceImpl implements ISpecializationService {

	@Autowired
	private SpecializationRepository repo;

	@Override
	public Long saveSpecialization(Specialization spec) {
		return repo.save(spec).getId();
	}

	@Override
	public List<Specialization> getAllSpecializations() {
		return repo.findAll();
	}

	@Override
	public void removeSpecialization(Long id) {
		// repo.deleteById(id); // before Exception handling we declare this we write
		repo.delete(getOneSpecialization( id));//after exception class declare we write
	}

	//before exception class we wrrote
	/*
	 * @Override public Specialization getOneSpecialization(Long id) {
	 * Optional<Specialization> optional = repo.findById(id);
	 * if(optional.isPresent()) { return optional.get(); } else { return null; } }
	 */

	// after exception class
	@Override 
	public Specialization getOneSpecialization(Long id) {
		Optional<Specialization> optional = repo.findById(id);
		if(optional.isPresent())
		{ return optional.get(); }
		else
		{
			throw  new SpecializationNotFoundExcetion(id+" Not Found");
		}
	}
	//java 8 without optional class throw the exception code

	/*return repo.findById(id).orElseThrow(
	()-> new SpecializationNotFoundException(id + " Not Found")
	);*/

	@Override
	public void updateSpecialization(Specialization spec) {
		repo.save(spec);
	}

	@Override
	public boolean isSpecCodeExist(String specCode) {
		/*Integer count = repo.getSpecCodeCount(specCode);
			boolean exist = count>0 ? true : false;
			return exist;*/
		//OR shortcut code
		return repo.getSpecCodeCount(specCode)>0;
	}

	@Override
	public boolean isSpecNameExist(String specName) {

		return repo.getSpecNameCount(specName)>0;
	}



}
